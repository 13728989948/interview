 ...
 * @Nam:layim v2.0.83
 * @Author:贤心
 * @Modifier:李大林(古雨,微信或Qq494174519)
...
layui.define(['jquery', 'layer', 'laytpl', 'upload'], function(exports){
  ...
  LAYIM.prototype.config = function(options){
    var skin = [
      layui.cache.dir+'images/skin/skin_001.jpg'
      ,layui.cache.dir+'images/skin/skin_002.jpg'
      ,layui.cache.dir+'images/skin/skin_003.jpg'
      ,layui.cache.dir+'images/skin/skin_004.jpg'
    ];
    options = options || {};
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'{{# layui.each(d.base.tabBar, function(index, item){ }}'
    ,'<li class="layui-icon {{# if(index==0){ }} layim-this {{# } }}" title="{{ item.title }}" layim-event="tab" lay-type="{{ item.type }}">{{ item.icon }}</li>'
    ,'{{# }); }}'
    ,'</ul>'
    // ,'<ul class="layui-layim-tab{{# if(!d.base.isfriend || !d.base.isgroup){ }}'
    //   ,' layim-tab-two'
  ...
  // 系统消息模板  wjq 2016-8-3
  var messageTpl = ['<li>'
  ,'<div style="height: 100px;background: #eee;width: 450px;"><h3>标题</h3><div>消息内容消息内容消息内容消息内容消息内容消息内容消息内容消息内容消息内容消息内容消息内容</div></div>'
  ,'</li>'].join('');
  ...
  var changeChat = function(elem, del){
    ...
    _tips(elem, "del");
    var classTmp = elem.attr("class");
    var classZero = (classTmp && classTmp.split(" ")[0]);
    var _o = (classZero && classZero.replace("chatlist-",""));
    _o && _tips($("#"+_o), "del");
  ...
  var anim = 'layui-anim-up', events = {
    ...
    ,tab: function(othis){
      ...
      index > 2 ? tabs.removeClass(THIS) : (
        ...
        ,othis.addClass(THIS).siblings().removeClass(THIS)
        ,_tips(othis,"del") //othis.find("em").remove()  //  点击查看主页面板tab切换消息，气泡消失--- wjq 2016-8-3
      )
  ...
  'modules/layim/layim.css?v=2.083'
  ,'skinlayimcss'
);